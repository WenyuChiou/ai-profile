...
needed by the user's configured remote are allowed. Before a push-capable
...
retry record is written privately before the forward branch CAS. A push
...
state still match its exact contract. If the original process stopped before
...
real index, and clears the record only after the remote is confirmed at the
commit.
