> **HISTORICAL RECORD** — this is the archived v0.1 run log. Statements
> here reflect the state at the time they were written (e.g. metric
> names, license status) and are NOT current; see `docs/progress.md`
> and `docs/ROADMAP.md` for the present state.
