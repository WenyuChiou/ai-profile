...
Pins the v0.7.0 candidate contract, inheriting the prior Gate S from
...
PINNED_WHEEL_SHA256 = "e8b568e011055c6cb8b3baaadb647cbd338bbcb82a37465c5ae46f6f41757740"
...
def test_workflow_verifies_the_exact_candidate_digest_before_pages_upload():
    ...
    assert "--expected-version 0.7.0" in text
    assert 'manifest["package_version"] == "0.7.0"' in text
...
def test_workflow_renders_in_and_uploads_only_a_fresh_verified_staging_root():
    ...
    assert '--out "$STAGING_ROOT/v0.7.0"' in text
    assert "path: ${{ runner.temp }}/aiprofile-staging\n" in text
    assert text.count("assert root.is_dir() and not root.is_symlink()") == 2
    assert text.count('("v0.7.0", "dir")') == 2
    assert text.count('("v0.7.0/dashboard.html", "file")') == 2
    assert text.count('("v0.7.0/staging-manifest.json", "file")') == 2
