...
Pins the v0.6.1 candidate contract, inheriting the prior Gate S from
...
#: The frozen v0.6.1 candidate digest (docs/reviews/promotion-candidate.json).
PINNED_WHEEL_SHA256 = "6ca24828fbba02024904028fa8fa5f96e97a8393d3f5e16bb6ff316cff477b9f"
...
def test_workflow_verifies_the_exact_candidate_digest_before_pages_upload():
    ...
    assert "--expected-version 0.6.1" in text
    assert 'manifest["package_version"] == "0.6.1"' in text
...
def test_workflow_renders_in_and_uploads_only_a_fresh_verified_staging_root():
    ...
    assert '--out "$STAGING_ROOT/v0.6.1"' in text
    assert "path: ${{ runner.temp }}/aiprofile-staging\n" in text
    assert text.count("assert root.is_dir() and not root.is_symlink()") == 2
    assert text.count('("v0.6.1", "dir")') == 2
    assert text.count('("v0.6.1/dashboard.html", "file")') == 2
    assert text.count('("v0.6.1/staging-manifest.json", "file")') == 2
